...
def TagConfig():
    #Keyword colors
    textPad.tag_config("Token.Keyword",foreground="blue");
    textPad.tag_config("Token.Keyword.Type",foreground="DodgerBlue3");
    textPad.tag_config("Token.Comment.Preproc",foreground="sienna");
    textPad.tag_config("Token.Comment.PreprocFile",foreground="sienna");
    textPad.tag_config("Token.Literal.Number",foreground="dark orange");
...
def SaveAs():
    ...
    p = tkFileDialog.asksaveasfilename(defaultextension=".cs",filetypes=types);
    ...
    filePath = p.replace('/','\\'); #Save the path to filePath
...
def Run(event):
    ...
    executable = filePath.split('.')[0].replace('/','\\') + ".exe";
...
def Highlight():
    ...
    for token, content in lex(data, CSharpLexer()):
